...
func UpdateSampleObj(kubeconfig *string, namespace string, group string, version string, resource string, step string, output string) {
	config, err := clientcmd.BuildConfigFromFlags("", *kubeconfig)
	if err != nil {
		panic(err)
	}
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		result := GetObj(kubeconfig, namespace, group, "v1", resource, "workflow-sample-01")
		steps, found, err := unstructured.NestedSlice(result.Object, "spec", "steps")
		if err != nil || !found || steps == nil {
			panic(fmt.Errorf("steps not found or error in spec: %v", err))
		}
